...
export const downloadBulkTemplate = async (req, res, next) => {
  try {
    ...
    const baseHeaders = [
      'sku', 'name', 'category', 'hsnCode', 'description',
      'currentStock', 'unit', 'purchasePrice', 'sellingPrice',
      'gstRate', 'reorderPoint', 'warehouseName', 'locationName'
    ];
    ...
    if (templateId) {
      ...
      if (template?.customColumns?.length) {
        customColumnNames = template.customColumns
          .filter(c => c.name?.trim())
          .map(c => c.name.trim());
      }
    }

    const allHeaders = [...baseHeaders, ...customColumnNames];
    const extendedRows = sampleRows.map(row => [
      ...row,
    ...
    const csvLines = [
      allHeaders.join(','),
      ...extendedRows.map(row => row.map(v => '"' + (v || '').replace(/"/g, '""') + '"').join(','))
    ];

    const filename = templateId ? 'bulk_inventory_template.csv' : 'bulk_inventory_sample.csv';
...
// One-time migration: moves currentStock into locationStock for old items
export const migrateToLocationStock = async (req, res, next) => {
  try {
    ...
    res.json({ success: true, message: `Migrated ${migrated} items to locationStock`, migrated });
  } catch (error) { next(error); }
};
